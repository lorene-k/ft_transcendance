...
export class Room {
    ...
    public constructor (mode: string, player1: Player, player2: Player) {
        console.log(`🎮 Création d'une Room en mode "${mode}" entre ${player1.username} et ${player2.username}`);
       ...
       private checkMatchStatus(): void {
        ...
        this.statusIntervalId = setInterval(() => {
            ...
            if (this.gameLogic.player1Score >= 7 || this.gameLogic.player2Score >= 7) {
                console.log("je passe par checkMatchSatus et je vais appeler endMatch")
                this.endMatch();
                clearInterval(this.statusIntervalId!);
                this.statusIntervalId = null;
            }
    ...
    // Détermination du rôle du joueur qui appelle (si userId fourni)
    if (userId) {
...
private endMatch(): void {
    ...
    console.log(`🏁 Fin de match - Mode: ${this.mode}, MatchType: ${this.matchType}, TournamentId: ${this.tournamentId}`);
